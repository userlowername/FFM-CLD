...
class CustomViT(nn.Module):
    def __init__(self, model_path):
        ...
        self.vit = ViTModel.from_pretrained(model_path,
                                            ...
                                            num_labels=0,)
        self.dropout = nn.Dropout(p=0.1)  # 假设原代码中有Dropout层

    def forward(self, x):
        outputs = self.vit(x)  # 输出类型为BaseModelOutputWithPooling
        # 提取CLS Token特征（等价于pooler_output）
...
def get_backbone(name, preserve_readout, pretrained):
    ...
    # [#] VIT models
        ...
        n_outputs = 768
    # elif name == "vit-base":
    #     network = CustomViT("/media/vipsl04/Harddisk/Deep-Fake-Detector-v2-Model")
    #     #network.head = nn.Identity()
    #     n_outputs = 768
    elif name == "vit-small":
